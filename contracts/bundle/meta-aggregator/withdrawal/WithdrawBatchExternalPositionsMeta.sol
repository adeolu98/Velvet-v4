...
/**
 * @title WithdrawBatchExternalPositionsMeta
 ...
 */
contract WithdrawBatchExternalPositionsMeta is ReentrancyGuard {
  ...
  function multiTokenSwapAndWithdraw(
    ...
    for (uint256 i = 0; i < swapTokenLength; i++) {
      ...
      if (_tokenToWithdraw == _token) {
        ...
        TransferHelper.safeApprove(_token, SWAP_TARGET, 0);
        TransferHelper.safeApprove(_token, SWAP_TARGET, _swapAmounts[i]);

        (bool success, ) = SWAP_TARGET.call(_callData[i]);
