 ...
 */
contract DepositBatchMetaAggregator is ReentrancyGuard {
  ...
  function _multiTokenSwapAndDeposit(
    ...
    if (data._depositToken != ETH_ADDRESS) {
      TransferHelper.safeApprove(_depositToken, SWAP_TARGET, 0);
    ...
    for (uint256 i; i < tokenLength; i++) {
      ...
      if (_token == _depositToken) {
        ...
        (bool success, ) = swapTarget.call{ value: data._ethSwapAmounts[i] }(
          data._callData[i]
        );
