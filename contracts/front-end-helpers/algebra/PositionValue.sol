...
library PositionValue {
  ...
  function total(
    ...
    (uint256 amount0Fee, uint256 amount1Fee) = _fees(
      positionManager,
      _position
    );
  ...
  function _getPosition(
    INonfungiblePositionManager positionManager,
    uint256 tokenId
  ) private returns (PositionCache memory) {
    (
      ,
      address token0,
      address token1,
    ...
    returns (uint256 feeGrowthInside0X128, uint256 feeGrowthInside1X128)
  {
    ...
    (
      uint256 lowerOuterFeeGrowth0Token,
      uint256 lowerOuterFeeGrowth1Token
    ) = _getOuterFeeGrowth(pool, bottomTick);
    (
      uint256 upperOuterFeeGrowth0Token,
      uint256 upperOuterFeeGrowth1Token
    ) = _getOuterFeeGrowth(pool, topTick);
      ...
      if (currentTick < topTick) {
        if (currentTick >= bottomTick) {
          ...
          feeGrowthInside0X128 =
            ...
            lowerOuterFeeGrowth0Token;
          feeGrowthInside1X128 =
            feeGrowthGlobal1X128 -
            lowerOuterFeeGrowth1Token;
        } else {
  ...
  function _getOuterFeeGrowth(
    ...
    private
    returns (uint256 outerFeeGrowth0Token, uint256 OuterFeeGrowth1Token)
  {
